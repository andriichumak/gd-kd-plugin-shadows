...
2.  Develop your plugin code in `src/dp_kd_plugin_shadow`

    The `src/dp_kd_plugin_shadow/Plugin.tsx` is the main plugin file where you have to register all
    your custom content. However, you can create as many new files as you want under the `src/dp_kd_plugin_shadow`
...
5.  Add plugin to one or more workspaces: `npm run add-plugin -- <url>`
    ...
    Run the `npm run add-plugin -- "https://your.hosting/pluginDirOfYourChoice/dp_dp_kd_plugin_shadow.js"` to
...
6.  Use plugin on a dashboard: `npm run link-plugin -- <plugin-object-id>`
    ...
    If your plugin supports parameterization (see [src/dp_kd_plugin_shadow](./src/dp_kd_plugin_shadow/Plugin.tsx)) and
...
This project is setup so that all your custom code must be self-contained in the [src/dp_kd_plugin_shadow](./src/dp_kd_plugin_shadow) directory.

The [src/dp_kd_plugin_shadow\_engine](./src/dp_kd_plugin_shadow_engine) and [src/dp_kd_plugin_shadow\_entry](./src/dp_kd_plugin_shadow_entry) directories contain essential plugin boilerplate.
